...
if ( vamtam_has_woocommerce() ) {
	...
	function vamtam_woocommerce_cart_item_quantity( $content, $cart_item_key, $cart_item ) {
		...
		if ( ! $only_one_allowed && is_cart() ) {
			$select_cart_attrs = 'name="cart[' . esc_attr( $cart_item_key ) . '][qty]" value="' . esc_attr( $cart_item['quantity'] ) . '" title="' . esc_attr__( 'Qty', 'bijoux' ) . '" min="0" max="' . esc_attr( $_product->get_max_purchase_quantity() ) . '"';
		}
